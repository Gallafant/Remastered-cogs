...
class GAR:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.file_path = "data/GAR/settings.json"
    ...
    async def _verify_json(self, e, *a, **k):
        ...
        if "ENABLED" not in _k:
            self._set_default(s)
            print("Please stop messing with the GAR JSON\n")
            return
        if "ROLE" not in _k:
            self._set_default(s)
            print("Please stop messing with the GAR JSON\n")
    ...
    @commands.group(name="GAR", pass_context=True, no_pm=True)
    async def GAR(self, ctx):
        """Change settings for GAR
...
def check_folders():
    if not os.path.exists("data/GAR"):
        print("Creating data/GAR folder...")
        os.makedirs("data/GAR")


def check_files():

    f = "data/GAR/settings.json"
    if not dataIO.is_valid_json(f):
        print("Creating default GAR settings.json...")
        dataIO.save_json(f, {})
