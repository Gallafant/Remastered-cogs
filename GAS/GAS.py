...
class GAS:  # TODO: Rewrite the whole thing. No easier way out of this.
    """Prevent people from sending messages too fast in channels"""
    
    DATA_FOLDER = "data/GAS"
...
get deleted **if** it's within the last {messages}. Don't worry, this won't get you a warning.\n\n"""
    ...
    def __init__(self, bot: discord.Client):
        self.bot = bot
        self.logger = logging.getLogger("red.ZeCogs.GAS")
        self.check_configs()
        self.load_data()
    ...
    async def GAS(self, ctx, channel: discord.Channel=None,
    ...
    @GAS.command(name="example", pass_context=True)
    @checks.mod_or_permissions(manage_channels=True)
    async def GAS_example(self, ctx):
...
At 70 and 72 seconds, you can see the effect of the `max_time` parameter""")
    ...
    @checks.mod_or_permissions(manage_channels=True)
    async def check_user_GAS(self, ctx, channel: discord.Channel):
    ...
    @check_user_GAS.command(name="all", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_channels=True)
    async def check_all_slows(self, ctx):
        """Checks and fixes all GASs in the server"""
    ...
    async def superior(self, ctx, channel: discord.Channel, *, role: discord.Role):
    ...
    @superior.command(name="all", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_roles=True)
    ...
    @superior.command(name="remove", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_roles=True)
    async def remove(self, ctx, channel: discord.Channel, *, role: discord.Role):
    ...
    @superior.command(name="list", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_roles=True)
    async def listroles(self, ctx, channel: discord.Channel):
