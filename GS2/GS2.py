...
class Feeds(object):
    def __init__(self):
        ...
        self.feeds = fileIO("data/GS2/feeds.json", "load")

    def save_feeds(self):
        fileIO("data/GS2/feeds.json", "save", self.feeds)

    def check_folders(self):
        if not os.path.exists("data/GS2"):
            print("Creating data/GS2 folder...")
            os.makedirs("data/GS2")
        self.check_files()

    def check_files(self):
        f = "data/GS2/feeds.json"
...
class GS2(object):
    ...
    async def GS2(self, ctx):
        """GS2 feed stuff"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)

    @GS2.command(pass_context=True, name="add")
    ...
    @GS2.command(pass_context=True, name="list")
    ...
    @GS2.command(pass_context=True, name="template")
    async def template(self, ctx, feed_name: str, *, template: str):
    ...
    @GS2.command(pass_context=True, name="force")
    async def force(self, ctx, feed_name: str):
        """Forces a feed alert"""
    ...
    @GS2.command(pass_context=True, name="remove")
    ...
    async def read_feeds(self):
        ...
        while self == self.bot.get_cog('GS2'):
            feeds = self.feeds.get_copy()
            for server in feeds:
