...
class SupportMe:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.settings = dataIO.load_json('data/RM/SupportMe/settings.json')
        self.set_server = self.bot.get_channel(self.settings["channel"])
        self.ignored_users = dataIO.load_json("data/RM/SupportMe/ignoredlist.json")
        ...
        if isinstance(message.channel, discord.PrivateChannel):
            ...
            if react == "reply":
                ...
                try:
                    ...
                    print("Couldn't clear reactions (No Permissions)")
    ...
    async def R(self, ctx, user: discord.Member=None, *, message):
        ...
        if member:
            ...
            try:
                ...
                await self.bot.say("No channel has been set-up.")
            except discord.errors.Forbidden:
    ...
    @checks.admin_or_permissions(manager_server=True)
    @S.command(pass_context=True)
    async def I(self, ctx, user: discord.Member, *, reason=None):
        """Add a user to the ignore list.

        Example: [p]SI @BakersBakeBread Spamming"""
        if user.id not in self.ignored_users["ignored"]:
            if reason is None:
                reason = "Reason not specified"
            self.ignored_users["ignored"].append(user.id)

            dataIO.save_json("data/RM/SupportMe/ignoredlist.json", self.ignored_users)
        ...
        try:
            await self.bot.delete_message(ctx.message) # delete command
        except discord.errors.Forbidden:
            pass

    @checks.admin_or_permissions(manager_server=True)
    @S.command(pass_context=True)
    async def C(self, ctx, channel:discord.Channel=None):
        """Set the channel SupportMe will be sent too"""

        if channel is None:
            channel = ctx.message.channel
        self.settings["channel"] = channel.id
        self.set_server = self.bot.get_channel(self.settings["channel"])
        dataIO.save_json("data/RM/SupportMe/settings.json", self.settings)
        try:
            await self.bot.send_message(channel, "I will send SupportMe here.")
        except discord.errors.Forbidden:
    ...
    @checks.admin_or_permissions(manager_server=True)
    @S.command(pass_context=True)
    async def UI(self, ctx, user: discord.Member):
    ...
    def _makelist(self, _list):
        ...
        return []
def check_folder():
    if not os.path.exists('data/RM/SupportMe'):
        print('Creating RM/SupportMe folder...')
        os.makedirs('data/RM/SupportMe')

def check_files():
    if not os.path.exists("data/RM/SupportMe/ignoredlist.json"):
        print("Creating empty ignores.json...")
        data = {"ignored": []}

        dataIO.save_json("data/RM/SupportMe/ignoredlist.json", data)
    if not os.path.exists("data/RM/SupportMe/settings.json"):
        print("Creating settings.json...")
        data = {"channel": " "}

        dataIO.save_json("data/RM/SupportMe/settings.json", data)

def setup(bot):
    check_folder()
    check_files()
    bot.add_cog(SupportMe(bot))
