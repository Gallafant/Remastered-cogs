...
class GRA:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.settings = dataIO.load_json("data/GRA/settings.json")
    ...
    async def unrole(self, ctx, role:discord.Role):
        ...
        if ctx.message.server.id not in self.settings:
            await self.bot.say("Please use the rules set command to change the role removed")
            return
        self.settings[ctx.message.server.id]["unrole"] = role.id
        await self.bot.say("Role removed {}".format(role.name))
        self.savefile()
    ...
    async def on_member_join(self, member):
        ...
        if answer.reaction.emoji == "🇾":
            ...
            await self.bot.remove_roles(member, unrole)
...
def check_folder():
    if not os.path.exists("data/GRA"):
        print("Creating data/acceptrules folder")
        os.makedirs("data/GRA")


def check_file():
    data = {}
    f = "data/GRA/settings.json"
