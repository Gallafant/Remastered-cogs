...
class GAL:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.location = 'data/GAL/settings.json'
    ...
    @GAL.command(pass_context=True, no_pm=True)
    ...
    @GAL.group(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(administrator=True)
...
def check_folder():
    if not os.path.exists('data/GAL'):
        os.makedirs('data/GAL')


def check_file():
    f = 'data/GAL/settings.json'
