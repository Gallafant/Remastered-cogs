...
class GS3:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.twitch_streams = dataIO.load_json("data/GS3/twitch.json")
        self.hitbox_streams = dataIO.load_json("data/GS3/hitbox.json")
        self.mixer_streams = dataIO.load_json("data/GS3/beam.json")
        self.picarto_streams = dataIO.load_json("data/GS3/picarto.json")
        settings = dataIO.load_json("data/GS3/settings.json")
    ...
    async def twitch_alert(self, ctx, stream: str):
        ...
        dataIO.save_json("data/GS3/twitch.json", self.twitch_streams)
    ...
    async def hitbox_alert(self, ctx, stream: str):
        ...
        dataIO.save_json("data/GS3/hitbox.json", self.hitbox_streams)
    ...
    async def mixer_alert(self, ctx, stream: str):
        ...
        dataIO.save_json("data/GS3/beam.json", self.mixer_streams)
    ...
    async def picarto_alert(self, ctx, stream: str):
        ...
        dataIO.save_json("data/GS3/picarto.json", self.picarto_streams)
    ...
    async def twitchtoken(self, token : str):
        ...
        dataIO.save_json("data/GS3/settings.json", self.settings)
...
def check_folders():
    if not os.path.exists("data/GS3"):
        print("Creating data/GS3 folder...")
        os.makedirs("data/GS3")
...
def check_files():
    ...
    for filename in stream_files:
        if not dataIO.is_valid_json("data/GS3/" + filename):
            print("Creating empty {}...".format(filename))
            dataIO.save_json("data/GS3/" + filename, [])

    f = "data/GS3/settings.json"
